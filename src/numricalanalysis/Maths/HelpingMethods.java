...
public class HelpingMethods {
    public static double[] generateDouble(String equation) {
        for (double i = 0.000; i < 10; i = i + .001) {

            if ((calculateValue(toFuctions(equation), i) > 0 && calculateValue(toFuctions(equation), i + .001) < 0)||(calculateValue(toFuctions(equation), i) < 0 && calculateValue(toFuctions(equation), i + .001) > 0)) {
                return new double[]{i, i + .001};
            }
        }
